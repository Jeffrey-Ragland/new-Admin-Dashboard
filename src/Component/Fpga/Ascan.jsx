...
const Ascan = () => {
  ...
  const apifun = async () => {
    ...
    try {
      const drp_data = await axios.get(
        "http://localhost:4000/sensor/ASCAN_PROJECT_LIST"
      );
      if (drp_data.data) {
      ...
      if (parseInt(btn_sts) == 1) {
        try {
          const ascan_resposne = await axios.get(
            `http://localhost:4000/sensor/BPCL_READ?`
          );
          if (ascan_resposne.data) {
            setAscan(ascan_resposne.data);
            console.log("ascan response", ascan_resposne);
          }
        } catch (error) {
      ...
      console.log("error from api fun:", error);
    }
  };
  ...
  const StartButton = async () => {
    ...
    if (value == "None") {
      ...
      const fulldate = yy + "-" + dd + "-" + mm + "," + time;

      try {
        console.log("fulldate = ", fulldate);
        const ascan_resposne = await axios.get(
          `http://localhost:4000/sensor/BPCL_READ?fromdate=${fulldate}&project=${value}`
        );
  ...
  const final_ascan_data = ascan_data
    ? ascan_data.success == false
      ? []
      : ascan_data.data
    : [];
    ...
    try {
      if (projectName) {
        const response = await fetch(
          "http://localhost:4000/sensor/ASCAN_PROJECT_ADD",
          {
            method: "POST",
            headers: {
  ...
  const projectDeleteButton = async () => {
    if (value1 == " ") {
      ...
      const response = await fetch(
        "http://localhost:4000/sensor/ASCAN_PROJECT_DELETE",
        {
          method: "POST",
          headers: {
            "Content-type": "application/json",
  ...
  const submitbutton = async () => {
    if (
      ...
      if (value != "None") {
        const response = await fetch(
          "http://localhost:4000/sensor/SETTINGS_PAGE",
          {
            method: "POST",
            headers: {
              "Content-type": "application/json",
      ...
      <div className="h-[10%] flex">
        ...
        <div className="flex justify-center items-center w-[92%]">
          <h2 className="font-bold text-xl text-white">
            XYMA HARDWARE CONTROL SYSTEM
          </h2>
        </div>
        <div
          className="flex justify-center items-center text-2xl mr-2 font-bold hover:cursor-pointer hover:scale-110 duration-200"
          onClick={Settings_Page}
        >
        ...
        <div className="bg-[#faf7f5] w-[100%] xl:w-[20%] h-[30%] xl:h-[100%] rounded-lg">
          ...
          <div className="h-[93%] p-4 overflow-auto">
            <div className="font-bold flex justify-center items-center mb-4">
              <div className="border-b-2 border-[#8a4300] text-sm">
            ...
            <div className="flex justify-end items-end mt-2 ">
              <button
                className="font-bold text-sm border rounded-lg bg-[#2d2d2d] p-2 text-white hover:cursor-pointer hover:scale-110 duration-200"
        ...
        <div className="bg-[#faf7f5] w-[100%] mt-2 xl:mt-0 xl:w-[80%] h-[70%]  xl:h-[100%] rounded-lg">
          <div className="bg-[#c5c5c5] h-[7%] w-full flex justify-between items-center rounded-lg">
            ...
            <div className="flex gap-2 mr-2">
              ...
              <button
                className={` ${
                  Btn_Status == 1 ? "bg-[#01b3a0] text-white" : ""
                } border font-bold text-xs p-1 border-black hover:bg-[#01b3a0] hover:text-white`}
                onClick={StartButton}
              >
                START
              </button>
              <button
                className={`${
                  Btn_Status == 0 ? "bg-[#dd0404] text-white" : ""
                } border font-bold text-xs p-1 border-black hover:bg-[#dd0404] hover:text-white`}
                onClick={StopButton}
              >
                CLEAR
              </button>
      ...
      {setting_Page && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70">
          <div className="bg-white p-6 rounded shadow-lg w-[80%] h-[80%] text-center">
            <div
              className="flex justify-end items-end text-xl"
              onClick={settingCancelPage}
            >
            ...
            <div className="grid xl:grid-cols-3 gap-2 w-full h-[100%]">
                ...
                <div className="h-[100%] w-[100%] p-4">
                  ...
                  <div className="flex h-[6%]">
                    ...
                    <div className="mr-2">
                      <select
                        value={value1}
                        className="border bg-blue-400 border-black text-white rounded-r-md"
              ...
              <div className="border w-full h-[100%]">cols-2</div>
              <div className="border w-full h-[100%]">cols-3</div>
            </div>
          </div>
        </div>
      )}
